package com.monster.makeover.ads.admob


//real ad ids
/*
object AdmobConstant {
    const val APP_ID = "ca-app-pub-9919447440379444~4494133729"
    const val APP_OPEN = "ca-app-pub-9919447440379444/3907864261"
    const val INTERSTITIAL_START_CREATE = "ca-app-pub-9919447440379444/9471128238"
    const val INTERSTITIAL_CREATE_END = "ca-app-pub-9919447440379444/8458301601"
    const val REWARDED_COINS = "ca-app-pub-9919447440379444/1473272617"
    const val NATIVE = ""
    const val BANNER_START = "ca-app-pub-9919447440379444/6945091144"
    const val BANNER_END = "ca-app-pub-9919447440379444/5915026602"
    const val COUNTER_TIME = 3L
}
*/

//test ad ids
object AdmobConstant {
    const val APP_OPEN = "ca-app-pub-3940256099942544/3419835294"
    const val INTERSTITIAL_START_CREATE = "ca-app-pub-3940256099942544/1033173712"
    const val INTERSTITIAL_CREATE_END = "ca-app-pub-3940256099942544/1033173712"
    const val REWARDED_COINS = "ca-app-pub-9919447440379444/1473272617"
    const val NATIVE = ""
    const val BANNER_START = "ca-app-pub-3940256099942544/6300978111"
    const val BANNER_END = "ca-app-pub-3940256099942544/6300978111"
    const val COUNTER_TIME = 3L
}